...
def create_download_report(patterns, type_profile, type_detail, Long, largeur_totale=None, 
    ...
    if is_surface_optim and type_profile == "Tôle/Platine":
        title = "Rapport d'optimisation de découpe de tôle"
    else:
        title = "Rapport d'optimisation de découpe de barres"
    ...
    story.append(Paragraph(material_info, normal_style))
    story.append(Spacer(1, 12))
    ...
    if is_surface_optim and type_profile == "Tôle/Platine":
        data = [
            ["Nombre total de plaques", f"{len(patterns)}"],
            ["Taux de perte (%)", f"{taux_perte:.2f}"],
            ["Taux d'efficacité (%)", f"{taux_efficacite:.2f}"]
        ]
    else:
        total_barres = len(patterns)
        total_coupe = sum(sum(pattern['cuts']) for pattern in patterns) if 'cuts' in patterns[0] else 0
        total_waste = sum(pattern.get('waste', pattern.get('waste_length', 0)) for pattern in patterns)
        
        data = [
            ["Nombre total de barres", f"{total_barres}"],
            ["Total des chutes (mm)", f"{total_waste}"],
            ["Taux de perte (%)", f"{taux_perte:.2f}"],
            ["Taux d'efficacité (%)", f"{taux_efficacite:.2f}"]
        ]
    ...
    # Generate pie chart and save to buffer for embedding in PDF
    if taux_perte is not None and taux_efficacite is not None:
        # Create a pie chart using Plotly
        import plotly.graph_objects as go
        fig_pie = go.Figure(data=[go.Pie(
            labels=['Utilisé', 'Perte'],
            values=[taux_efficacite, taux_perte],
            marker_colors=['#2ca02c', '#707070']
        )])
        fig_pie.update_layout(
            title="Répartition de l'utilisation des matériaux",
            width=500,
            height=300
        )
    ...
    if unique_patterns:
        ...
        for i, unique_pattern in enumerate(unique_patterns):
            pattern_title = f"Motif #{i+1} - Utilisé {unique_pattern['count']} fois"
            story.append(Paragraph(pattern_title, styles["Heading3"]))
            
            if 'pattern' in unique_pattern and isinstance(unique_pattern['pattern'], list):
                # For linear profiles (UPN, Cornière)
                pattern_details = ", ".join([f"{x} mm" for x in unique_pattern['pattern']])
                waste = unique_pattern.get('waste', 0)
                
                pattern_info = f"Composition: {pattern_details}, Chute: {waste} mm"
                story.append(Paragraph(pattern_info, normal_style))
            elif 'pattern' in unique_pattern and isinstance(unique_pattern['pattern'], dict):
                # For surface optimization (Tôle/Platine)
                h = unique_pattern['pattern']['h']
                v = unique_pattern['pattern']['v']
                waste_pct = unique_pattern.get('waste_percentage', 0)
                
                pattern_info = f"Disposition: {h} × {v} pièces, Total: {h*v} pièces, Chute: {waste_pct:.2f}%"
                story.append(Paragraph(pattern_info, normal_style))
