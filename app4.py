...
def create_download_report(patterns, type_profile, type_detail, Long, largeur_totale=None, 
                           ...
                           epaisseur=None, is_surface_optim=False):
    ...
    # -------------------------------
    # Tableau principal de statistiques
    # -------------------------------
    stat_data = []
    stat_data.append(['Statistiques', 'Valeurs'])
    
    if is_surface_optim:
        # Surface optimization statistics
        total_surface_nette = sum([p.get('surface', 0) for p in patterns]) if patterns else 0
        if largeur_totale:
            total_surface_utilisee = Long * largeur_totale
        else:
            # Approximation si largeur_totale n'est pas fournie
            total_surface_utilisee = total_surface_nette * (1 + taux_perte/100 if taux_perte else 1)
        
        stat_data.append(['Surface nette (mm²)', f"{int(total_surface_nette)}"])
        stat_data.append(['Surface utilisée (mm²)', f"{int(total_surface_utilisee)}"])
        if largeur_totale:
            stat_data.append(['Dimensions totales', f"{Long} x {largeur_totale} mm"])
        stat_data.append(['Nombre de plaques', f"{len(patterns)}"])
        stat_data.append(['Taux de chute', f"{taux_perte:.1f}%"])
        stat_data.append(['Taux d\'efficacité', f"{taux_efficacite:.1f}%"])
    else:
        # Length optimization statistics
        total_length_net = sum([sum(p.get('cuts', [])) for p in patterns]) if 'cuts' in patterns[0] else sum([sum([s.get('length', 0) for s in p.get('segments', [])]) for p in patterns])
        total_waste = sum([p.get('waste', p.get('waste_length', 0)) for p in patterns])
        
        stat_data.append(['Métrage net (mm)', f"{int(total_length_net)}"])
        stat_data.append(['Métrage utilisé (mm)', f"{int(Long * len(patterns))}"])
        stat_data.append(['Nombre de barres', f"{len(patterns)}"])
        stat_data.append(['Total des chutes (mm)', f"{int(total_waste)}"])
        stat_data.append(['Taux de chute', f"{taux_perte:.1f}%"])
        stat_data.append(['Taux d\'efficacité', f"{taux_efficacite:.1f}%"])
    
    # Create the statistics table with blue header
    stat_table = Table(stat_data, colWidths=[doc.width/2-10, doc.width/2-10])
    ...
    # -------------------------------
    # Tableau des longueurs demandées
    # -------------------------------
    if not is_surface_optim and patterns:
        # Collect all segments across all patterns
        all_segments = []
        for pattern in patterns:
            if 'segments' in pattern:
                all_segments.extend(pattern['segments'])
            elif 'cuts' in pattern:
                # If using the old 'cuts' format, convert to segments format
                pos = 0
                for i, cut in enumerate(pattern['cuts']):
                    segment = {
                        'type': type_detail,
                        'length': cut,
                        'position': pos
                    }
                    # Try to get name if it exists in a matching format
                    if 'names' in pattern and i < len(pattern['names']):
                        segment['name'] = pattern['names'][i]
                    all_segments.append(segment)
                    pos += cut
        
        # Create dictionary to count occurrences of each length with name
        length_counts = {}
        for segment in all_segments:
            # Determine the key for grouping
            if 'name' in segment and segment['name']:
                # Use the name provided by user with pattern type
                key = f"{segment['name']} {segment.get('type', type_detail)}"
            else:
                # Just use the pattern type and length
                key = f"{segment.get('type', type_detail)} {segment['length']}mm"
            
            if key not in length_counts:
                length_counts[key] = {'count': 0, 'length': segment['length']}
            length_counts[key]['count'] += 1
        
        # Convert to table data
        lengths_data = [['Identifiant', 'Longueur (mm)', 'Quantité']]
        for name, info in length_counts.items():
            lengths_data.append([name, f"{info['length']}", f"{info['count']}"])
    ...
    if not is_surface_optim:
        # Information about the bars used
        bars_data = [['Barre', 'Longueur (mm)', 'Quantité']]
        bars_data.append([f"{type_profile} {type_detail}", f"{Long}", f"{len(patterns)}"])
        
        bars_table_title = Paragraph("Détail des barres utilisées", subtitle_style)
        story.append(bars_table_title)
        
        bars_table = Table(bars_data, colWidths=[doc.width/3-10, doc.width/3-10, doc.width/3-10])
    ...
    # -------------------------------
    chart_title = Paragraph("Répartition de l'utilisation des matériaux", subtitle_style)
    ...
    colors = ['#4CAF50', '#AAAAAA']  # Green for used, Grey for waste
